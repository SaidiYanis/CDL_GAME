...
export function AgeDuelScreen({ players, teams }: AgeDuelScreenProps) {
  ...
  const handleSubmitAnswer = useCallback(
    (answer: DuelAnswer) => {
      setGameState((currentState) =>
        submitAgeDuelAnswer(currentState, players, answer),
      );
    },
    [players],
  );
  ...
  const handleRestartGame = useCallback(() => {
    setGameState(startAgeDuelGame(players, gameState.bestScore));
      ...
      <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
        <section className="mx-auto max-w-3xl rounded-[2rem] border border-white/10 bg-white/5 p-8">
          ...
          <h1 className="mt-5 text-4xl font-black tracking-[-0.04em]">
            Pas assez de joueurs avec une date de naissance valide.
          </h1>
        </section>
      </main>
    );
